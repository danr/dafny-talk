...
method VectorUpdate(a : array<A>, f : (int,A) -> A)
  requires a != null;
...
{
  var v := new int[10];
  VectorUpdate(v, (i,_) => i);
  PrintArray(v);
  VectorUpdate(v, (_,x) => x + 1);
  PrintArray(v);
  VectorUpdate(v, (_,x) requires x != 0 => 100 / x);
  PrintArray(v);
  var u := new int[10];
  VectorUpdate(u, (i,_) requires 0 <= i < 10 reads v => v[i]);
  PrintArray(u);
  var z := new int[9];
  VectorUpdate(z, (i,_) requires 0 <= i < 9 reads u => u[i] + u[i+1]);
  PrintArray(z);
  assert z[8] == 21; 
}
