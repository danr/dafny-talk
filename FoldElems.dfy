...
function Elems(xs : List<A>): set<A>
  ensures forall x :: x in Elems(xs) ==> x < xs
{
  match xs
    case Nil => {}
...
function method Fold(xs : List<A>, unit : B, f : (A,B) -> B): B
  reads f.reads;
  requires forall x, y :: x in Elems(xs) ==> f.requires(x,y);
{
  match xs
    case Nil => unit
...
{
  match e
    case Add(es) => Fold(es,0,(e',v) requires e' in Elems(es) => Eval(e') + v)
    case Mul(es) => Fold(es,1,(e',v) requires e' in Elems(es) => Eval(e') * v)
    case Lit(i)  => i
}
